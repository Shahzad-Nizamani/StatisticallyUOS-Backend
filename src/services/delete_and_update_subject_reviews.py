...
def update_review(review_id, rating, review_msg):
    ...
    db_session = session()
    review = db_session.execute(text("select * from subject_review where id = :review_id"), {"review_id" :review_id}).fetchone()
    ...
    if not review:
        ...
        update_query = "UPDATE subject_review SET rating = :rating, review_msg = :review_msg WHERE id = :review_id"
...
def delete_review(review_id):
    ...
    if review:
        delete_query = "DELETE FROM subject_review WHERE id =:review_id"
